...
void app_main(void) {
    ...
    ESP_LOGI("CS1237_DEBUG", "Cấu hình hiện tại trong chip là: 0x%02X", current_cfg);
    ESP_LOGI("CS1237_DEBUG", "Cấu hình hiện tại trong chip ph la: 0x%02X", current_cfg_ph);
    ...
    while (1) {
        ...
        // 3. IN KẾT QUẢ
        ESP_LOGI(TAG, "Nhiệt độ: %.2f C (ADC Temp: %" PRId32 ") | ADC pH: %8" PRId32 " | V_Probe: %7.2f mV | pH (ATC): %.2f", current_temp, temp_filtered, ph_filtered, v_probe_mv, current_ph);
        // ESP_LOGI(TAG, "Temp raw: %" PRId32 " Nhiệt độ: %.2f C (ADC Temp: %" PRId32 ")", temp_raw, current_temp, temp_filtered);
